...
on run argv
	...
	-- set yesterday to "1" as string
	set yesterday to {query} as string
	log yesterday
	if yesterday is "1" then
		set day_ to current date
		set startDate to day_ - days
		set endDate to day_
	else
		set day_ to current date
		set startDate to day_
			...
			repeat with a from 1 to length of theProjects
				...
				if dropTasks is not equal to {} then
					...
					repeat with b from 1 to length of dropTasks
						...
						set drop_text to drop_text & return & "- [ ] [" & name of currentTask & "](" & omnifocusURL & ")  ?" & time_str
